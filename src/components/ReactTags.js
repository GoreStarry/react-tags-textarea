...
class ReactTags extends Component {
	static propTypes = {
		...
		inputType: PropTypes.oneOfType([PropTypes.string, PropTypes.object]), // PropTypes.oneOf(['input', 'textarea']),
	...
	handleBlur(event) {
		...
		if (this.props.isWhenBlurAutoUpdate) {
			if (event.target.value.trim()) {
				this.props.onTagUpdate(currentEditIndex, {
					[this.props.labelField]: event.target.value,
				});
			}
		}
